...
lookup_exn :: (Show a, Eq a) => a -> [(a, t)] -> t
lookup_exn v as = f (lookup v as)
    where
      f (Just x) = x
      f Nothing = error ("can't find module" ++ (show v))
...
expand (Pair p1 p2) (Times t1 t2, env) =
    [Pair v1 v2 | v1 <- expand p1 (t1, env),
                  v2 <- expand p2 (t2, env)]
expand (LeftE p) (Plus t1 _, env) =
    [LeftE v | v <- expand p (t1, env)] ++ [RightE (Var "_")]
expand (RightE p) (Plus _ t2, env) =
...
-- if they will satisfy exactly the same values, then they are pureed.
-- if one is more general than the other, then it is split up into
--   more specific subpatterns
-- if they can't be reconciled then no patterns are pureed
reconcile :: PVal -> PVal -> CurTyp -> ([PVal], [PVal])
reconcile x y   _ | matchesAny x && matchesAny y = ([x], [y])
...
reconcile (LeftE p1) (LeftE p2) (Plus t1 _, env) =
    ([LeftE v | v <- r1], [LeftE v | v <- r2])
    where (r1, r2) = reconcile p1 p2 (t1, env)
reconcile (RightE p1) (RightE p2) (Plus _ t2, env) =
...
covers _ [] [] _ = []
covers msg [] ps _ = map (MultipleMatches msg) ps
covers msg ps [] _ = map (NoMatches msg) ps
...
exhaustive context ps ct =
    ...
    if debug
    then trace ("| Checking " ++ context ++ " : " ++ (show ps')) res else res
...
tProg defs = loop defs emptyEnv
    where
      ...
      loop ((Import _):defs') env = loop defs' env -- ignoring imports for now
      loop ((Eval _ _):defs') env = loop defs' env -- ignoring the main
      loop ((Iso name _ ityp labels clauses):defs') env =
          let types = packTypes ityp labels in
          (process name clauses types env (\(Clause lhs _) -> lhs) (\(ITyp t1 _)->t1) "LHS")
          ++ (process name clauses types env (\(Clause _ rhs) -> rhs) (\(ITyp _ t2)->t2) "RHS")
          ++ (loop defs' env)
